...
import { lostService } from '../services/api';
import { useToast } from '../context/ToastContext';
import { Upload, X, ArrowLeft, Send, Sparkles, HelpCircle } from 'lucide-react';
...
export const ReportLost = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.location.trim()) newErrors.location = 'Lost location is required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await lostService.create(submitData);
      if (result.success) {
        showToast('Lost item reported. Matches are being calculated!', 'success');
        navigate('/');
      } else {
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-xl md:text-2xl font-extrabold text-slate-100 flex items-center gap-2">
            Report Lost Item <HelpCircle className="w-6 h-6 text-rose-400" />
          </h1>
          <p className="text-xs text-slate-400">File a report for an item you lost on campus</p>
        </div>
      </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Space Gray iPad Pro"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="e.g., Library 2nd floor, Main Gym, Cafeteria"
                  className={`w-full bg-slate-950 border rounded-xl py-3 px-4 text-slate-100 placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all ${
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`w-full bg-slate-950 border rounded-xl py-3 px-4 text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all cursor-pointer ${
                    errors.date_lost ? 'border-rose-500 focus:ring-rose-500' : 'border-slate-800 focus:border-indigo-500'
                  }`}
                />
                {errors.date_lost && <p className="text-rose-500 text-xs mt-1 font-medium">{errors.date_lost}</p>}
              </div>
            </div>
            ...
            <div className="space-y-5 flex flex-col justify-between">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  placeholder="Provide distinct keywords e.g. serial number, keychains attached, stickers on cover, color details."
